...
int CHudMOTD :: Draw( float fTime )
{
	...
	int ypos = std::max(((ScreenHeight - (m_iLines * LINE_HEIGHT)) / 2) - 40, 30 ); // shift it up slightly
	char *ch = m_szMOTD;
	while ( *ch )
	{
...
int CHudMOTD :: MsgFunc_MOTD( const char *pszName, int iSize, void *pbuf )
{
	...
	if ( is_finished )
	{
		...
		MOTD_DISPLAY_TIME = std::max( 10, CVAR_GET_FLOAT( "motd_display_time" ) );
