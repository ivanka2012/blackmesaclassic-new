...
void CHAssassin :: RunAI( void )
{
	...
	if (pev->renderamt > m_iTargetRanderamt)
	{
		...
		pev->renderamt = std::max( pev->renderamt - 50.f, static_cast<float>(m_iTargetRanderamt) );
		pev->rendermode = kRenderTransTexture;
	}
	else if (pev->renderamt < m_iTargetRanderamt)
	{
		pev->renderamt = std::min( pev->renderamt + 50.f, static_cast<float>(m_iTargetRanderamt) );
		if (pev->renderamt == 255)
